...
def main(args):
    *_, func_args = inspect.getargvalues(inspect.currentframe())
    ...
    text_encoder = CLIPTextModel.from_pretrained(args.pretrained_model_path, subfolder="text_encoder").to("cpu")
    vae          = AutoencoderKL.from_pretrained(args.pretrained_model_path, subfolder="vae").to("cpu")
    ...
    for model_idx, model_config in enumerate(config):
        ...
        unet = UNet3DConditionModel.from_pretrained_2d(args.pretrained_model_path, subfolder="unet", unet_additional_kwargs=OmegaConf.to_container(inference_config.unet_additional_kwargs)).to("cpu")
        ...
        if model_config.get("controlnet_path", "") != "":
            ...
            controlnet_images = torch.stack(controlnet_images).unsqueeze(0).to("cpu")
